...
> ***Answer 3***:
The matrix is represented as "matrix[rows,columns]". By adding a - sign preceeding the the index of the row/column, the specified vector is discarded from the matrix
...
> Answer 4:
While the scatter plots help us understand the general trend in the relationship between two variables and visually seek out the outliers in the plot or any unusal clumping that may occur in the data, the correlation denotes the strength of relationship between the two variables. 
...
cor3 <- cor.test(meso, method = "kendall", alternative = "greater") #Kendall's correlation
cor4 <- cor.test(meso, method = "spearm", alternative = "g")#Spearman's correlation
...
# lines(newTN, regline)
# conf95 <- predict(fitreg, newdata = data.frame(TN = newTN), interval = c("confidence"), level = 0.95, type ="reponse")
...
arrows(x0 = bp, y0 = zp.means - zp.sem, angle = 90, length =0.1, lwd =1)
arrows(x0 = bp, y0 = zp.means + zp.sem, angle = 90, length =0.1, lwd =1)
