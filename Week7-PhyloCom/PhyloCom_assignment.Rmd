...
# Calculate NRI
NRI <- as.matrix(-1 * ((ses.mpd[,2] - ses.mpd[,3]) / ses.mpd[,4])) rownames(NRI) <- row.names(ses.mpd)
colnames(NRI) <- "NRI"
...
rownames(NTI) <- row.names(ses.mntd) colnames(NTI) <- "NTI"
...
par(mar = c(5, 5, 2, 1) + 0.1) plot(dist.mp, dist.uf,
pch = 20, col = "red", las = 1, asp = 1, xlim = c(0.15, 0.5), ylim = c(0.15, 0.5),
xlab = "Mean Pair Distance", ylab = "UniFrac Distance") abline(b = 1, a = 0, lty = 2)
text(0.5, 0.47, "1:1")
```
...
# Add Axes
axis(side = 1, labels = T, lwd.ticks = 2, cex.axis = 1.2, las = 1) axis(side = 2, labels = T, lwd.ticks = 2, cex.axis = 1.2, las = 1) abline(h = 0, v = 0, lty = 3)
box(lwd = 2)
# Add Points & Labels
...
ponds.fit <- envfit(ponds.dbrda, envs, perm = 999) ponds.fit
...
long.lat <- as.matrix(cbind(env$long, env$lat)) coord.dist <- earth.dist(long.lat, dist = TRUE)
...
unifrac.dist.ls <- liste(unifrac.dist, entry = "unifrac") bray.curtis.dist.ls <- liste(bray.curtis.dist, entry = "bray.curtis") coord.dist.ls <- liste(coord.dist, entry = "geo.dist")
...
DD.reg.bc <- lm(df$bray.curtis ~ df$geo.dist) summary(DD.reg.bc)
...
DD.reg.uni <- lm(df$unifrac ~ df$geo.dist) summary(DD.reg.uni)
...
diversity <- c(diversity, mean(diversity.iter)) # Let Diversity be the Mean PSV areas <- c(areas, mean(areas.iter)) # Let areas be the Average Area print(c(i, mean(diversity.iter), mean(areas.iter))) # Print As We Go
}
...
# Compute the PDAR
pdar <- PDAR(comm, phy)
pdar <- as.data.frame(pdar) pdar$areas <- sqrt(pdar$areas)
# Calculate Pearson's correlation coefficient
Pearson <- cor.test(pdar$areas, pdar$diversity, method = "pearson") P <- round(Pearson$estimate, 2)
P.pval <- round(Pearson$p.value, 3)
# Calculate Spearman's correlation coefficient
Spearman <- cor.test(pdar$areas, pdar$diversity, method = "spearman") rho <- round(Spearman$estimate, 2)
rho.pval <- round(Spearman$p.value, 3)
# Plot the PDAR
plot.new()
par(mfrow=c(1, 1), mar = c(1, 5, 2, 1) + 0.1, oma = c(2, 0, 0, 0)) plot(pdar[, 1], pdar[, 2], xlab = "Area", ylab = "PSV", ylim = c(0, 1),
